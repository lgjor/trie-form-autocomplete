...
**Por onde começar**:
1. Implementar `loadFromArray()` (mais simples)
2. Implementar `getData()`
3. Implementar `addData()`
4. Implementar `loadFromAPI()` (usar fetch)
5. Implementar `loadFromFile()` (se necessário)
...
**Por onde começar**:
1. Implementar `indexData()` - popular a Trie
2. Implementar `search()` básico
3. Adicionar opções (limite de resultados)
4. Implementar cache (opcional)
5. Implementar filtros customizados
...
**DOMUtils - Prioridade**:
1. `createElement()` - criar elementos HTML
2. `removeAllChildren()` - limpar elementos
3. `highlightMatch()` - destacar texto
4. `addDebouncedListener()` - debounce
5. `calculateDropdownPosition()` - posicionamento

**Validator - Prioridade**:
1. `isValidString()`
2. `isValidArray()`
3. `normalizeString()` - importante para busca
4. `validateConfig()`
...
**Por onde começar**:
1. Implementar `create()` - criar elemento UL
2. Implementar `render()` - renderizar itens
3. Implementar `show()` e `hide()`
4. Implementar navegação (`selectNext()`, `selectPrevious()`)
5. Implementar `getSelectedItem()`
